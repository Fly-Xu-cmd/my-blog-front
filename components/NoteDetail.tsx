...
/**
 * 笔记详情客户端组件
 * @param slug 笔记的唯一短链接标识
 */
export default function NoteDetail({ slug }: { slug: string }) {
      ...
      <div className="max-w-[1240px] mx-auto flex flex-col lg:flex-row items-start gap-8 px-4 py-8 w-full">
        ...
        >
          <header className="mb-8 border-b pb-6 dark:border-white/10">
            ...
            <div className="flex flex-wrap gap-4 text-gray-500 dark:text-white/50 text-sm">
              <Space split={<span className="text-gray-300 dark:text-white/20">|</span>}>
                    ...
                    {note.tags.map(
                      (
                        ...
                        return (
                          <span
                            key={idx}
                            className="bg-blue-50 text-blue-600 dark:bg-cyan-400/10 dark:text-cyan-300 px-2 py-0.5 rounded-md text-xs"
                          >
                            #{tag}
                          </span>
                        );
          ...
          <div
            ref={containerRef}
            className="w-full text-gray-800 dark:text-white/80 leading-relaxed"
          >
